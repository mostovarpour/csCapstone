...
GByte *datasetread(GDALImage *image, int width, int height)
{
    struct timeb start;
    ftime(&start);
    GByte *buffer = CPLMalloc(sizeof(GByte) * image->band_count * width * height);
    GDALResult r;
    r = GDALDatasetRasterIO(image->dataset, GF_Read, 0, 0, image->original_width, image->original_height,
            buffer, width, height, GDT_Byte, image->band_count, NULL, 0, 0, 0);
    struct timeb end;
    ftime(&end);
    printf("Time: %lldms\n", compare_time(&start, &end));
    if(r == CE_Failure)
